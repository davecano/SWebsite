...
namespace Z
{
    ...
    public class Utils
    {
        ...
        /// <param name="dp">下拉框编号</param>
        /// <param name="value">默认绑定值</param>
        public static void BindDropDownList(Type type, DropDownList dp, string value)
        {
            dp.Items.Clear();
            dp.Items.Add(new ListItem("==请选择==",""));
            FieldInfo[] fields = type.GetFields();
            for (int i = 1; i < fields.Length; i++)
            {
                dp.Items.Add(new ListItem(fields[i].Name));
            }
            if (value != "")
            {
                dp.SelectedIndex = dp.Items.IndexOf(dp.Items.FindByValue(value));
            }
        ...
        {
            double size = double.Parse(osize.ToString());
            double mod = 1024*1024;
            size /= mod;
