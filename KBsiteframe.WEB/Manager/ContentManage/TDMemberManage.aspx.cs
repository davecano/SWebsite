...
namespace KBsiteframe.WEB.Manager.ContentManage
{
    ...
    public partial class TDMemberManage : PageBase
    {
        public TDMemberManage()
        {
        ...
        private void BindingList()
        {
            ...
            qm.Add("MemberType", MemberType.团队成员.ToString());
        ...
        {
            Response.Redirect("TDMemberAdd.aspx");
        }
        ...
        protected void rplist_OnItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "bj")
            {
                Response.Redirect("TDMemberEdit.aspx?ID=" + e.CommandArgument);
            }
            if (e.CommandName == "sc")
            {
                ...
                if (bm.Delete(newsmodel) == 1)
                {
                    ...
                    log.OperateUser = GetLogUserName();
                    log.OperateDate = DateTime.Now;
                    log.LogOperateType = "删除团队成员信息";
                    log.LogBeforeObject = JsonHelper.Obj2Json(newsmodel);
                    bsol.Insert(log);

                    Message.ShowOK(this, "删除团队成员信息成功!");
                }
          
                else
                    Message.ShowWrong(this, "删除团队成员信息失败");
